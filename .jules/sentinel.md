...
## 2024-05-03 - Fix SSRF and Path Traversal in generate-image Function
**Vulnerability:** The `resolveImageUrls` function in Edge Functions lacked input validation for user-supplied URLs and Supabase storage paths. HTTP/HTTPS URLs were not checked against metadata/private IP ranges (SSRF), and storage paths did not prevent directory traversal or cross-user access (IDOR).
**Learning:** Edge Functions with broad permissions (e.g., using `SUPABASE_SERVICE_ROLE_KEY` to access storage buckets and bypassing RLS) are highly susceptible to SSRF and IDOR if inputs are not strictly validated. Additionally, standard String methods like `startsWith` are insufficient for URL validation; `URL.hostname` should be parsed to catch IP literals like `169.254.169.254` or `0x7f000001`.
**Prevention:** Always parse untrusted URLs using the native `URL` class and validate the host against disallowed ranges. For storage paths manipulated by privileged service roles, enforce constraints preventing path traversal sequences (`../`) and ensuring paths match expected ownership prefixes (e.g., `${userId}/`).
