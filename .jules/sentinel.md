...
## 2025-05-02 - SSRF and Path Traversal Mitigations in Storage Functions
**Vulnerability:** Edge function `generate-image` implicitly trusted user-provided inputs for image resolution, allowing potential SSRF via HTTP URLs targeting internal IPs (e.g., `169.254.169.254`, `localhost`, `127.0.0.1`, `10.0.0.1`) or obfuscated IP formats (like `2130706433`), and Path Traversal via storage paths containing `../` sequences in `supabase.storage.from()`.
**Learning:** `new URL(input).hostname` effectively resolves obfuscated IP formats to standard dotted-decimal notation. Without first asserting that the host is an IP literal (`/^(\d{1,3}\.){3}\d{1,3}$/.test(host)`), simple `startsWith` checks on hostnames can cause false positives (e.g., blocking `10.example.com`). Also, client-provided file paths to internal storage APIs must be strictly validated.
**Prevention:** Always parse untrusted URLs using the native `URL` class and check against known bad IP ranges after confirming the host is an IP literal. Explicitly check for and reject path traversal sequences (like `../` or `..\`) in inputs mapped to backend storage.
