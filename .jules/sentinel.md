...
## 2025-04-25 - Prevent SSRF and Path Traversal via User-Controlled URLs
**Vulnerability:** The image generation edge function accepted raw `imageInputs` string URLs and paths directly. It allowed generating arbitrary signed URLs including path traversal vectors like `../` to access unauthorized storage paths. It also blindly resolved full HTTP(s) URLs without validating the target hostname, exposing the server to SSRF where an attacker could force the server to fetch metadata via `169.254.169.254` or internal services via `127.0.0.1`.
**Learning:** Differentiating between protocol-less strings (as storage paths) and HTTP URls is fragile without validation. If parsing external URL inputs, they must be validated to prevent SSRF using native classes (like `URL`) which normalize formats natively, thus mitigating evasion by IP formatting (e.g. integer `2130706433`).
**Prevention:** Always explicitly check the parsed `hostname` against blocked internal CIDR blocks and IP forms before performing outbound server requests. For local file or storage paths, explicitly reject traversal primitives like `../` and `..\` before acting on the path.
