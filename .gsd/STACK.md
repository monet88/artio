...
| purchases_flutter | ^9.0.0 | RevenueCat in-app purchases (planned) |
| google_mobile_ads | ^6.0.0 | AdMob ads (planned) |
...
| Database | Supabase (Postgres) | Templates, profiles, generation_jobs |
| Object Storage | Supabase Storage | Generated images (bucket: `generated-images`) |
| Edge Functions | Supabase (Deno) | Server-side image generation orchestration |
| AI Generation (primary) | KIE.ai | Imagen4, Imagen4-Fast, Imagen4-Ultra, Nano-Banana, Flux-2, GPT Image, Seedream |
| AI Generation (secondary) | Google Gemini | gemini-3-pro-image-preview, gemini-2.5-flash-image |
| Error Tracking | Sentry | Production crash reporting |
| In-App Purchases | RevenueCat | iOS/Android subscriptions (planned) |
| Payments | Stripe | Web payments (planned) |
| Ads | Google AdMob | Mobile ads (planned) |
...
| `GEMINI_API_KEY` | Google Gemini API key | `.env.*` + Supabase secrets |
| `KIE_API_KEY` | KIE.ai API key | `.env.*` + Supabase secrets |
| `SENTRY_DSN` | Sentry error tracking DSN | `.env.*` files |
| `REVENUECAT_APPLE_KEY` | RevenueCat Apple API key | `.env.*` files (planned) |
| `REVENUECAT_GOOGLE_KEY` | RevenueCat Google API key | `.env.*` files (planned) |
| `REVENUECAT_WEB_KEY` | RevenueCat Web API key | `.env.*` files (planned) |
| `STRIPE_PUBLISHABLE_KEY` | Stripe publishable key | `.env.*` files (planned) |
...
| flutter test | `flutter test` | Run unit + widget tests |
| integration tests | `flutter test integration_test/` | Run E2E tests |
| analysis | `flutter analyze` | Static analysis (very_good_analysis) |
...
|--------|---------|
| `master` | Main branch |
| `feature/admin-dashboard-ui` | Admin dashboard UI (current HEAD) |
| `feature/ui-ux-overhaul` | UI/UX redesign |
| `fix/ios-admob-config` | iOS AdMob configuration fix |
| `plan-2026-02-14-text-to-image-plan` | Text-to-image feature planning |
| `plan-2026-02-14-text-to-image-plan-dev2` | Text-to-image dev iteration |
